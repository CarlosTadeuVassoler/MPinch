...
def preparar_dados_e_rede():
	...
	for quente in range(nhot):
		temperatura_atual_quente[quente] = float('{:.1f}'.format(pinchq))

	for fria in range(ncold):
		temperatura_atual_fria[fria] = float('{:.1f}'.format(pinchf))
...
def adicionar_utilidade(dlg, corrente):
	...
	if Qutilcold[ccoldutil-1] == 0:
		QMessageBox.about(dlg, "Error!", "The heat of this stream has already been supplied")
		return
	Tcfinal0[ccoldutil-1] = Tcf[ccoldutil-1]
	temperatura_atual_fria[ccoldutil-1] = float('{:.1f}'.format(Tcf[ccoldutil-1]))
	calor_atual_frio[ccoldutil-1] = 0.0
	utilidades.append([ccoldutil, float('{:.1f}'.format(Qutilcold[ccoldutil-1]))])
...
def remover_utilidade(corrente, indice_remover, utilidades):
	...
	calor_atual_frio[corrente-1] = float('{:.1f}'.format(Qtotalc0[corrente-1][0][0]))
	temperatura_atual_fria[corrente-1] = float('{:.1f}'.format(-calor_atual_frio[corrente-1] / CPc[corrente-1] + Tcf[corrente-1]))
