...
def inserir_trocador(vetor):
	...
	if Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] > Qmax:
		...
		print('A quantidade de calor trocada foi 0.')										#usuário quer continuar. Se quiser, descarta
	#for si in range(ncold):
	...
	#trocamax = input('Deseja trocar o calor Máximo? ').strip().upper()[0]
		...
		#Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] = float(input('Quanto calor será trocado: '))
			...
			#input()
							...
							#calor inválido < 0, descarta o que foi
...
def recriar_rede(matriz):			#fiz isso pq a interface nao armazena a temperatura de todos os sk e k
	for trocador in matriz:
		...
		k = trocador[5]
		q = trocador[6]
		inserir_trocador([chot, ccold, si, sj, sk, k, q])
	return linha_interface
