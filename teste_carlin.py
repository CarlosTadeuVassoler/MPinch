...
def preparar_dados_e_rede2():
	...
	for quente in range(nhot):
		temperatura_atual_quente_abaixo[quente] = float('{:.1f}'.format(pinchq))

	for fria in range(ncold):
		temperatura_atual_fria_abaixo[fria] = float('{:.1f}'.format(pinchf))
...
def inserir_trocador_abaixo(vetor):
	cont = 0
	...
	linha_interface_abaixo.append([chot,
							...
							float('{:.1f}'.format(Thskf[chot-1][sbhot-1][sestagio-1][estagio-1])),
							float('{:.1f}'.format(Tcskf[ccold-1][sbcold-1][sestagio-1][estagio-1])),
							fracao_quente,
							fracao_fria])
...
inserir_trocador_abaixo([1,2,1,1,1,1,30])
inserir_trocador_abaixo([2,2,1,1,1,1,10])
print(linha_interface_abaixo)

def remover_trocador_abaixo(vetor, indice, linha_interface_abaixo):
	chot = vetor[0]
	ccold = vetor[1]
	sbhot = vetor[2]
	sbcold = vetor[3]
					...
					for j in range(ncold):
						for sj in range(nhot):
							...
							if Qaux[i][si][j][sj][sk][k] != 0:
								...
								temperatura_atual_quente_abaixo = Thout[i][si][j][sj][sk][k]
								...
								temperatura_atual_fria_abaixo = Tcout[i][si][j][sj][sk][k]
...
#			print()
	...
	if calor_atual_frio_abaixo[ccold-1] == Q_frio_da_corrente_sem_troca_abaixo[chot-1]:
...
def adicionar_utilidade_abaixo(corrente):
	chotutil = corrente
	Qutilhot[chotutil-1] = Qtotalh0[chotutil-1][0][0]
	for si in range(ncold):
		for k in range(nstages):
			if Qutilhot[chotutil-1] < Qtotalh0[chotutil-1][si][k]:
				Qutilhot[chotutil-1] = Qtotalh0[chotutil-1][si][k]
			Qtotalh0[chotutil-1][si][k] = 0
	if Qutilhot[chotutil-1] == 0:
		return
	Thfinal0[chotutil-1] = Thf[chotutil-1]
	temperatura_atual_quente_abaixo[chotutil-1] = float('{:.1f}'.format(Thf[chotutil-1]))
	calor_atual_quente_abaixo[ccoldutil-1] = 0.0
	utilidades_abaixo.append([chotutil, float('{:.1f}'.format(Qutilhot[chotutil-1]))])
...
def remover_utilidade_abaixo(corrente, indice_remover, utilidades_abaixo):
	...
	calor_atual_quente_abaixo[corrente-1] = float('{:.1f}'.format(Qtotalh0[corrente-1][0][0]))
	temperatura_atual_quente_abaixo[corrente-1] = float('{:.1f}'.format(-calor_atual_quente_abaixo[corrente-1] / CPh[corrente-1] + Thf[corrente-1]))
	utilidades_abaixo.pop(indice_remover)

def caixa_de_temperatura_abaixo():
	pass
