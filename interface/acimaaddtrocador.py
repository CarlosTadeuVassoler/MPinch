...
def acimaaddtrocador(correntestrocador,pinchq,pinchf,n,nhot,ncold,dlg,dTmin,estagio,chot,qsi,fracao_quente,qsj,fracao_fria,ccold,ntrocadores,nlinhas,Qtabela0,opcao):
    ...
    if nlinhas >= 2:
        for i in range (nlinhas-1):
            ...
            Qtotalh0[chot-1][sbhot-1][estagio-1] = int(float(dlg.tableWidget_3.item(i,3).text()))
            Qtotalc0[ccold-1][sbcold-1][estagio-1] = int(float(dlg.tableWidget_4.item(i,3).text()))
                    ...
                    for j in range(ncold-1, -1, -1):
                        for sj in range(nhot-1, -1, -1):
                            ...
                            if Qaux[i][si][j][sj][sk][k] != 0:
                                ...
                                if dlg.radioButton.isChecked() or dlg.radioButton_4.isChecked():  #Heat Load ou Maximum Heat
                                ...
                                if tempdif >= dTmin:
                                    ...
                                    #('Erro! A diferença mínima de temperatura não está sendo respeitada: ', tempdif)  #ERRO!
                                    avanc = str(input('Deseja continuar mesmo assim? ')).strip().upper()[0]     #Caixa de erro, perguntar se deseja continuar.
                                    if avanc == 'Y':                                                         #se a pessoa clicar em yes.
                                        Tcfinal01[j][sj] = Tcout[i][si][j][sj][sk][k]
                                        Thfinal01[i][si] = Thout[i][si][j][sj][sk][k]
                                        Thfinal01k[i][k] = Thoutk[i][si][j][sj][sk][k]
                                        Tcfinal01k[j][k] = Tcoutk[i][si][j][sj][sk][k]
                                        ...
                                        #('A troca de calor não ocorreu!')  #caso clique em no, na caixa de erro.

                                if Fharr[k][i][si] == 100:
                                    Fharr[k][i][si] = 0
                                if Fcarr[k][j][sj] == 100:
                                    Fcarr[k][j][sj] = 0
                                    #("Ajuda nois", Thski, Thskf)
    for k in range (nstages):
        for sk in range (nsk):
            for i in range (nhot):
                for si in range (ncold):
