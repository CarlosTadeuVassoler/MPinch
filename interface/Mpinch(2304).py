...
workbook = xlrd.open_workbook(r"C:\pinch5correntes.xlsx")
worksheet = workbook.sheet_by_index(0)
correntes = []
n = 5  # numero de correntes
...
P.subplot(121)
for i in range(2*n):
     P.plot([0, 10000], [Tdecre[i], Tdecre[i]], color = 'gray', linestyle = '--')
for i in range(n):
    if correntes[i][3] == 'quente':
        P.arrow(i + 1, correntes[i][0], 0.0, -dT[i], fc="r", ec="r", head_width=0.05, head_length=2)
    if correntes[i][3] == 'fria':
        P.arrow(i + 1, correntes[i][0], 0.0, -dT[i], fc="b", ec="b", head_width=0.05, head_length=2)
P.xlim([0, n + 1])
