...
Tc0 = [60, 60]
Tcf = [140, 170] #Temperaturas frias no pinch
CPc = [4, 2]
hc = [0.2, 0.2]
Thk = []
...
if ncold > nhot:
	...
	if somacphot > somacpcold:
		#("The CPhtotal must be smaller than the CPctotal.")
	else:
		Qtotalh01 = []
		Qtotalc01 = []
		Qtotalh0 = np.array([0])
		...
		while fim != no:
			if fim == "utilidade":
				...
				if(divisao == 'yes'):
					...
					if trocamax == 'no':
						Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] = input("Quanto calor será trocado: ")
								...
								for i in range(nhot) :
									for si in range(nsi[i]) :
										if Q[i][si][j][sj][sk][k] > Qmin  and Fh[i][si][sk][k] > Frmin and Fc[j][sj][sk][k] > Frmin :
											...
											Tcout[i][si][j][sj][sk][k] = Tcin[i][si][j][sj][sk][k] + Q[i][si][j][sj][sk][k] / (Fc[j][sj][sk][k] * CPc[j]) #Calcula-se a de saída
