...
def printar():
	...
	if len(matriz_armazenada) > 0:
		for trocador in range(0, len(matriz_armazenada)):
			...
			dlg.tableWidget_2.setItem(trocador, 6, QTableWidgetItem(str(matriz_armazenada[trocador][6]))) # calor trocado
			dlg.tableWidget_2.setItem(trocador, 7, QTableWidgetItem(str(matriz_armazenada[trocador][7]))) #Thin
			dlg.tableWidget_2.setItem(trocador, 8, QTableWidgetItem(str(matriz_armazenada[trocador][8]))) #Tcout
	...
	if len(utilidades) > 0:
		...
		for utilidade in range(0, len(utilidades)):
			...
			dlg.tableWidget_2.setItem(len(matriz_armazenada) + utilidade, 6, QTableWidgetItem(str(utilidades[utilidade][1])))

def inserir_teste():
	dados_do_trocador = ler_dados(dlg)
	print("trocador que vai inserir", dados_do_trocador)
...
def remover_teste():
	...
	if indice_remover <= len(matriz_armazenada) - 1:
		if len(utilidades) > 0:
			for i in range(len(Tc0), 0, -1):
				try:
					remover_utilidade(i, i-1, utilidades) #arrumar o indiceeeeeee pq ai nao da erro
				except:
					pass
			dlg.comboBox_10.setEnabled(False)
			dlg.pushButton_8.setEnabled(False)
...
def calcular_calor_teste():
	...
	dlg.TempLoad.pushButton_2.clicked.connect(lambda: dlg.TempLoad.close())
	dlg.TempLoad.pushButton.clicked.connect(lambda: caixa_de_temperatura(dlg))
...
def printar_abaixo():
	...
	for corrente in range(nhot):
		...
		dlg.tableWidget_15.setItem(corrente, 2, QTableWidgetItem(str(float('{:.1f}'.format(10)))))
	...
	for corrente in range(ncold):
		...
		dlg.tableWidget_17.setItem(corrente, 2, QTableWidgetItem(str(float('{:.1f}'.format(10)))))
	...
	if len(matriz_trocadores_abaixo) > 0:
		for trocador in range(0, len(matriz_trocadores_abaixo)):
			...
			dlg.tableWidget_14.setItem(trocador, 6, QTableWidgetItem(str(matriz_trocadores_abaixo[trocador][6]))) # calor trocado
			dlg.tableWidget_14.setItem(trocador, 7, QTableWidgetItem(str(matriz_trocadores_abaixo[trocador][7]))) #Thin
			dlg.tableWidget_14.setItem(trocador, 8, QTableWidgetItem(str(matriz_trocadores_abaixo[trocador][8]))) #Tcout
	...
	if len(utilidades_abaixo) > 0:
		...
		for utilidade in range(0, len(utilidades_abaixo)):
			...
			dlg.tableWidget_14.setItem(len(matriz_trocadores_abaixo) + utilidade, 6, QTableWidgetItem(str(utilidades_abaixo[utilidade][1])))
