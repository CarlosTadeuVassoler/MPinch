...
def printar():
	...
	for trocador in matriz_armazenada:
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,0,QTableWidgetItem(str(trocador[0]))) #chot
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,1,QTableWidgetItem(str(trocador[1]))) #ccold
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,2,QTableWidgetItem(str(trocador[2]))) #sbhot
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,3,QTableWidgetItem(str(trocador[3]))) #sbcold
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,4,QTableWidgetItem(str(trocador[4]))) #sk
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,5,QTableWidgetItem(str(trocador[5]))) #k
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,6,QTableWidgetItem(str(trocador[6]))) # calor trocado
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,7,QTableWidgetItem(str(trocador[7]))) #Thin
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,8,QTableWidgetItem(str(trocador[8]))) #Tcout
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,9,QTableWidgetItem(str(trocador[9]))) #fração hot
		dlg.tableWidget_2.setItem(len(matriz_armazenada)-1,10,QTableWidgetItem(str(trocador[10]))) #fraçao cold

	dlg.lineEdit_5.setText(str("0"))
	for corrente in range(nhot):
		dlg.tableWidget_3.setItem(corrente, 1, QTableWidgetItem(str(temperatura_atual_quente[corrente])))
		dlg.tableWidget_3.setItem(corrente, 3, QTableWidgetItem(str(calor_atual_quente[corrente])))
	for corrente in range(ncold):
		dlg.tableWidget_4.setItem(corrente, 1, QTableWidgetItem(str(temperatura_atual_fria[corrente])))
		dlg.tableWidget_4.setItem(corrente, 3, QTableWidgetItem(str(calor_atual_frio[corrente])))

def inserir_teste():
	dados_do_trocador = ler_dados(dlg)
	print(dados_do_trocador)
	nova_matriz = inserir_trocador(dados_do_trocador)
	matriz_armazenada.append(nova_matriz[-1])
	print(matriz_armazenada)
	atualizar_matriz(matriz_armazenada)
	printar()
	checaresgotadosacima() #libera pra adicionar utilidades
...
def remover_teste():
	...
	remover_trocador(trocador_remover, indice_remover, matriz_armazenada)
	atualizar_matriz(matriz_armazenada)
	print("nova matriz",matriz_armazenada)
	printar()
