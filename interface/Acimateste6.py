...
def acima (correntes,pinchq,pinchf,n,nhot,ncold,dlg,dTmin):
	Th0 = [] 
	Thf = []
	CPh = []
	Thm = []
	...
	if str(dlg.comboBox_4.currentText())!="do not split stream":
		sbcold=float(dlg.comboBox_4.currentText())
		...
		while fim != 'N':
			...
			if fim == 'U':
				...
				if(divisao == 'Y'):
					...
					if trocamax == 'N':
						Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] = float((dlg.lineEdit_5.text())) #dsijgdsiogj
					...
					if ((Qtotalh01[chot-1]) > (Qtotalc01[ccold-1])):
						print("Calor Máximo a ser trocado: ", Qtotalc01[ccold-1])
						Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] = Qtotalc01[ccold-1]
					else:
						print("Calor Máximo a ser trocado: ", Qtotalh01[chot-1])
				...
				if divisao == 'Y':
					...
					#print dos dados
					if divtype == 'Q':
						print("Temperatura da sub-corrente quente após a troca térmica")
						print(Thfinal01[chot-1][sbhot-1])
						print('Temperatura da corrente fria após a troca térmica')
						print(Tcfinal01[ccold-1][sbcold-1])
					if divtype == 'F':
						print("Temperatura da corrente quente após a troca térmica")
						print(Thfinal01[chot-1][sbhot-1])
						print('Temperatura da sub-corrente fria após a troca térmica')
						print(Tcfinal01[ccold-1][sbcold-1])
					if divtype == 'A':
						print("Temperatura da sub-corrente quente após a troca térmica")
						print(Thfinal01[chot-1][sbhot-1])
					...
					#print dos dados
					print("Carga térmica do trocador")
					print(Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1])
					print("Energia disponível das correntes após a troca")
					print(Qtotalh0)
					print(Qtotalc0)
					...
					if Thin[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] - Tcout[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] >= dTmin:
						...
						#print dos dados
						print("Temperatura da corrente quente após a troca:")
						print(Thfinal0[chot-1])
						print("Temperatura da corrente fria após a troca:")
						print(Tcfinal0[ccold-1])
						#Calor das Quentes
						Qtotalh0[chot-1][sbhot-1] = Qtotalh0[chot-1][sbhot-1] - Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1]
						Qtotalh0[chot-1][sbhot-1] = float("{:.2f}".format(Qtotalh0[chot-1][sbhot-1]))
						#Calor das Frias
						...
						print("Carga térmica do trocador:")
						print(Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1])
						print("Energia disponível nas correntes quentes após a troca:")
						print(Qtotalh0)
						print("Energia requerida pelas correntes frias após a troca:")
						print(Qtotalc0)
			...
			fim = "N"
			#return qual temperatura eu pego? começo ou fim do estagio?
