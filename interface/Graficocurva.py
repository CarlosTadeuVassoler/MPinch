...
def plotgrafcurva(corrente,dtmin,pinchf,pinchq,tf,tq):
    plt.close("all")
    ...
    plt.title(label='Cold pinch temperature: '+str(round(tf,5))+'°C\nHot pinch temperature: '+str(round(tq,5))+'°C\n', loc='center')

    ax.plot(hf,auxf, label='Cold balanced composite curves', color='b')
    ax.plot(pinchx,pinchy, label='Pinch: '+str(dtmin)+'°C', color='k')
    ax.set_xlabel('Q(kW)')
    ax.set_ylabel('T(ºC)')
    ax.plot(futx, futy, label='Cold utility: '+str(round(pinchf,2))+' kW', color='c')
    ax.plot(hutx, huty, label='Hot utility: '+str(round(pinchq,2))+' kW', color='orange')
    ax.plot(hq,auxq, label='Hot balanced composite curves', color='r')
