...
print("opopopopopopopop", somacpf,somacpq)
intervalosq = len(dHq)
intervalosf = len(dHf)
print("okkokokokokkoko",dHq)
somaq = 0
somaf = 0
somadH = 0
print(newcorrentesq[0][0])
print(pinchq - newcorrentesq[0][0] - (dTmin/2))
for i in range(int(pinchq - newcorrentesq[0][0] - (dTmin/2))):
    somadH = somadH + dHq[i]
    print("kiikikkikikki",somadH)
P.plot([0,0], [0,0], color = 'red')
P.plot([0,0], [0,0], color = 'blue')
...
P.ylabel('Temperature')
P.title('TH Diagram')
P.gca().legend((("hot utilities: {} kW".format(utilidadesquente)), ("cold utilities: {} kW".format(cascat2certo[2*n-2]))))
EixoY = []
EixoY.append(pinchq)
EixoY.append(pinchf)
...
EixoX.append(somaf)
EixoX.append(somaq)
print("opqepoqiopwi",somadH,somaq,somaf)
P.yticks(EixoY)
P.xticks(EixoX)
P.show()
cascat3 = []
...
for i in range(2*n-2):
    if i == 0:
        B = pd.Series({'Delta T': dTdecre[i+1], ' CPf-CPq': somacps[i+1], '   delta H': dHcerto[i+1]})
        A.append(B)
        df = pd.DataFrame([A[i]])
        print(df.to_string())
    else:
        B = pd.Series({'       ':dTdecre[i+1], '        ':somacps[i+1], '          ':dHcerto[i+1]})
        A.append(B)
        df = pd.DataFrame([A[i]])
        print(df.to_string())
print("------------------------------------------------------")
...
for i in range(2*n-1):
    if i == 0:
        D = pd.Series({'Interval': Tdecre[i+1], '      ': dHcerto[i], '   delta H': cascatcerto[i]})
        C.append(D)
        df1 = pd.DataFrame([C[i]])
        print(df1.to_string())
    else:
        D = pd.Series({'       ':Tdecre[i+1], '        ':dHcerto[i], '          ':cascatcerto[i]})
        C.append(D)
        df1 = pd.DataFrame([C[i]])
        print(df1.to_string())
print("------------------------------------------------------")
...
for i in range(2*n-1):
    if i == 0:
        F = pd.Series({'Interval': Tdecre[i+1], '      ': dHcerto[i], '   delta H': cascat2certo[i]})
        E.append(F)
        df2 = pd.DataFrame([E[i]])
        print(df2.to_string())
    else:
        F = pd.Series({'       ':Tdecre[i+1], '        ':dHcerto[i], '          ':cascat2certo[i]})
        E.append(F)
        df2 = pd.DataFrame([E[i]])
        print(df2.to_string())