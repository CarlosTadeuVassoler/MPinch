...
#(correntes,pinchq,pinchf,n,nhot,ncold)= ([[165.0, 55.0, 3.0, 'Hot'], [145.0, 25.0, 1.5, 'Hot'], [85.0, 145.0, 4.0, 'Cold'], [25.0, 140.0, 2.0, 'Cold']], 90.0, 80.0, 4, 2, 2)
...
def abaixo(correntes,pinchq,pinchf,n,nhot,ncold) :
	...
	Qtotalh = np.array([0])
	Qtotalh.resize(nhot)
	Qhotexcess = np.array([0])
	Qhotexcess.resize(nhot)
	...
	Qcoldlack = np.array([0])
	Qcoldlack.resize(ncold)
			...
			for sk in range(nsk) :
				for j in range(ncold) :
					for sj in range(nsj[j]) :
						for i in range(nhot) :
							for si in range(nsi[i]) :
								if Q[i][si][j][sj][sk][k] > Qmin  and (Fh[i][si][sk][k]/100) > Frmin and (Fc[j][sj][sk][k]/100) > Frmin:
									...
									Thout[i][si][j][sj][sk][k] = Thout[i][si][j][sj][sk][k] - (Q[i][si][j][sj][sk][k] / CPh[i]) #Calcula-se a de saída
									...
									Tcin[i][si][j][sj][sk][k] = Thin[i][si][j][sj][sk][k] - (Q[i][si][j][sj][sk][k] / CPc[j]) #Calcula-se a de saída
						...
						if (Fh[i][si][sk][k]/100) > Frmin :
							...
							if sk < nsk :
								...
								Thsk[i][si][sk][k] = Thsk[i][si][sk][k] + (sumQThsk[i][si][sk][k] / CPh[i])
							else :
								Thsfinal0[i][si][k] = Thsk[i][si][sk][k] + (sumQThsk[i][si][sk][k] / CPh[i])
						if (Fc[j][sj][sk][k]/100) > Frmin :
							if sk < nsk :
								#Se sk não for o último sub-estágio (neste caso, nsk)
								Tcsk[j][sj][sk][k] = Tcsk[j][sj][sk][k] + (sumQTcsk[j][sj][sk][k] / CPc[j])
							else :
								Tcsfinal0[j][sj][k] = Tcsk[j][sj][sk][k] + (sumQTcsk[j][sj][sk][k] / CPc[j])
