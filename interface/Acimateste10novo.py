...
def acima(correntes,pinchq,pinchf,n,nhot,ncold,dlg,dTmin,estagio,chot,qsi,fracao_quente):
	...
	for i in range (nhot):
		Qtotalh1 = 0
		print("printa ae palhaçoo ", CPh,Th0,Thf)
	...
	#def divisao_quente(estagio,chot,qsi,fracao_quente):
	...
	for i in range(qsi):      												#pq já vai tá o número máximo de correntes
		Fharr[estagio-1][chot-1][i] = fracao_quente[i]
...
#def divisao_fria(estagio,ccold,qsj,fracao_fria):     #if divtype == 'F'
	...
	def divisao_de_correntes():
		...
		#def divisao_quente(estagio,chot,qsi,fracao_quente):
			...
			qsi = int(float(dlg.divisaotrocador3.comboBox_2.currentText()))        #não precisa de janela de erro  
			for i in range(qsi):      												#pq já vai tá o número máximo de correntes
				Fharr[estagio-1][chot-1][i] = float(fracao_quente[i])
				Qtotalh0[chot-1][i] = Qtotalh01[chot-1]*(Fharr[estagio-1][chot-1][i]/100)
				print(Qtotalh0)
			nhotc = qsi + (nhot - 1) 
			print(nhotc)
	...
	#		if divop == 'Y':
	#divisao_de_correntes()                                         #criar janela de erro aqui, mas n entendi esse sys.exit
	#		else:
	#			break
	...
	if opcao == 1:
		...
		return(Fharr,nhotc)
			...
			#trocamax = input('Deseja trocar o calor Máximo? ').strip().upper()[0]  #relacionar ao maximum heat?

			if trocamax == 'N':
				Q[chot-1][sbhot-1][ccold-1][sbcold-1][sestagio-1][estagio-1] = float(dlg.lineEdit_5.currentText()) #heatload
			...
			if tempdif >= dTmin:
				...
				msg = QMessageBox()
				msg.setText("The minimum temperature difference is not being respected.")                            
